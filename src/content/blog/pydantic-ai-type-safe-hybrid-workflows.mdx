...
In [a previous post](https://danielfridljand.de/post/workflow-vs-agent) I argued that business automation should use LLMs only where they add irreplaceable value, with everything else in regular code. A mixed pipeline still needs clean seams between deterministic, LLM-assisted, and agentic steps. Pydantic models provide those seams.
...
The pipeline contract is the same in all three cases. Each step returns a Pydantic model, declared as `output_type` for the LLM-assisted and agentic variants. Whether the instance came from regular code, a single LLM call, or an agent that looped through five tool calls is local to that step.
...
The `output_type` on the error path is still `FetchResult`: we construct one with `record=None`, so downstream code doesn't need to special-case "did the agent work?" and sees the same shape either way. Every LLM call also gets an explicit timeout. An agent without a timeout is a production incident waiting to happen.
...
## What the shared contract buys you
