...
On a recent project, I had to classify thousands of customer support tickets, prepare training data, and keep every run reproducible. The expensive steps, mainly LLM calls and PII censoring, did not scale as a sequence of standalone scripts. I used [Snakemake](https://snakemake.readthedocs.io/) to orchestrate the pipeline. This post covers the pipeline and the three parts of Snakemake that helped most.

## The Challenge

I had over 2000 tickets exported from Zendesk. The goal was to classify each ticket by **customer intent** (e.g. billing questions, technical issues, feedback) so we could later build a first-level customer support AI agent. The classification was iterative: define intents, run an LLM over the tickets, inspect how many remained unclassified or misclassified, then refine the intent taxonomy and repeat. Without a workflow engine, each refinement would have required manual scripts, ad-hoc reruns, and duplicated work.
