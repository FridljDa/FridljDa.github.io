...
- Every pull request that gets opened automatically triggers a review pass by AI agents. Those reviewers feed their comments back into the agentic harness, which iterates on the code for, say, 30 rounds of back-and-forth, fixing clear mistakes and tightening obvious rough edges.
...
One caveat: the iterative loop does not yet close reliably. Agents still miss obvious bugs ten iterations in. The argument that follows is conditional on that gap closing. If you do not believe it ever will, none of what follows will apply.
...
GitHub's PR view is a diff with inline comments. That UI was built for a world where a human wrote those lines and another human inspected the patch before merge. Scrutinizing each line for off-by-ones and missed edge cases was the right job. In 2026 it is unfair to expect human reviewers to be that first reader, and it does not scale. The diff is the output. The interesting object is the process that produced it.

## Attach the conversation, not just the diff

### Steinberger's "prompt request"

Peter Steinberger, the founder of OpenClaw, has been arguing publicly for a while that the right unit of review in the age of agentic coding is no longer the diff but the prompt that produced it, which he calls a "prompt request" rather than a pull request ([Lex Fridman interview](https://lexfridman.com/peter-steinberger-transcript/)).
...
Attach the conversation history with the agentic coding harness to each commit: every prompt, tool call, rejected plan, test run, and resulting change in direction. Keep it alongside the code.
...
The diff is not going away; reviewers still approve or reject the code that will be merged. But review should start with the conversation, then move to the parts of the diff that need human attention.
