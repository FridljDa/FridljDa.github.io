...
The chat interface is one of the most interactive features of the site. It allows visitors to ask questions about my experience, skills, publication, and blog posts, with responses powered by Google's Gemini API.
...
1. **Home page content** - Biography, experience, publication, and blog post summaries
...
- `publicationToMarkdown()` - Converts publication to markdown
...
export function generateHomePageMarkdown(blogPosts: CollectionEntry<'blog'>[]): string {
  ...
  markdown += `---\n\n`;
  markdown += publicationToMarkdown();
  markdown += `---\n\n`;
  markdown += `# Blog Posts\n\n`;
