...
My [previous blog post](https://danielfridljand.de/post/mutational-signature-with-hierarchical-dirichlet-process/) derived a Hierarchical Dirichlet Process (HDP) for discovering mutational signatures, estimating their activities across samples, and learning the number of signatures from the data. That model shares information across samples but does not encode their evolutionary relationships.
...
<figure>
<img src="/images/blog/mapping-mutational-signatures-to-trees/example-tree.png"
 alt="300" />
...
Instead of making every sample depend directly on a common base distribution $G_0$, each tree node gets a Dirichlet Process based on its parent.
...
Because $G_4$ and $G_5$ vary around $G_3$, while $G_2$ and $G_3$ vary around $G_1$, the prior makes $G_5$ more similar to $G_4$ than to $G_2$. This induces the same relationship among $\color{#6672C9} e_{\cdot 5}$, $\color{#7630A9} e_{\cdot 4}$, and $\color{#BB6438} e_{\cdot 2}$ that motivated the extension.
...
The change is in the sampling of $G_j$. Consider a tree topology $\mathcal{T}$. We write $(v_i,v_s)\in \mathcal{T}$ if and only if the $i$-th node is a child of the $s$-th node. Then we model the Dirichlet Process at each node $v_j$ of the tree $\mathcal{T}$ as:
...
The current prototype is a fork of the [HDP implementation by Nicola Roberts](https://github.com/nicolaroberts/hdp) in R with some helper functions. A Python port would make it accessible to more users.
...
Instead of the Hierarchical Dirichlet Process approach, we could use variations of Non-negative Matrix Factorization. Given observed mutation catalogue $X$ and a constructed tree topology $\mathcal{T}$, we could introduce a penalization term $R_{\mathcal{T}}(E)$ to ensure that the tree topology is respected:
...
The proposed approach should also be compared with existing work such as [Alam et al.](https://doi.org/10.1007/978-3-319-99608-0_33) to identify its theoretical and practical differences.
