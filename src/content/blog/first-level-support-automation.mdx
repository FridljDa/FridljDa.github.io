...
Our production architecture for first-level customer support splits the problem into small steps, uses LLMs for interpretation, and keeps everything else in code. It is the production version of the approach described in [Workflow or Agent](https://danielfridljand.de/post/workflow-vs-agent).

The system turns a customer message into a reply, possibly with attachments, and any required tool calls: updating a record, filing a follow-up ticket, or escalating to a human. A preprocessing layer first normalises inbound attachments.
...
## The practical payoff: local evals
...
Typed steps make regressions easier to locate. Keeping known execution in code and routing coverage gaps through "other" makes the long tail manageable without turning the whole pipeline into an agent. The same pattern also applies to operations triage, legal intake, and insurance claims.
