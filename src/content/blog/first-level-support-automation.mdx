...
tags:
  ...
  - Customer Support
  - Architecture
description: "The four-step pipeline we landed on after iterating in production: a DAG of intents, grouped extraction, weighted-scoring fetch, and a mostly-hardcoded planner with an explicit LLM fallback."
...
Because intent classification already collapses multiple paths onto a shared leaf (Step 1), the planner branches on the sub-intent it landed on, not on which parent got it there — the `account locked` leaf is handled the same whether the customer came in via account access or card services.
