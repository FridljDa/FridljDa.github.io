...
Once we know the intent, we extract the structured information the downstream steps will need.
...
The last one is worth dwelling on. Throughout this pipeline we would rather over-extract than under-extract: the fetch step is built to assume that some of what came out of extraction is wrong, and to handle that with scoring rather than by trusting any single field.
...
The whole step is plain code. There is no LLM call here at all. It is also expressible in entity-resolution terms (a simplified probabilistic record linkage), but the mental model that matters in practice is just: fan out, sum weighted hits, threshold.
