...
Our production architecture for first-level customer support splits the problem into small steps, uses LLMs for interpretation, and keeps everything else in code. It is a worked example of [Agentic vs Workflow-based AI](https://danielfridljand.de/post/workflow-vs-agent).
...
Decompose into typed steps, reserve LLMs for interpretation, encode known execution in code, and make coverage gaps explicit with "other". This makes regressions easier to locate and keeps the long tail manageable. The same pattern also applies to operations triage, legal intake, and insurance claims.
