...
We now have all three ingredients: a softmax (6), a dot-product score $\tilde{a}$ (12), and a $1/\sqrt{d}$ rescaling. Computing attention for each query one at a time would work, but the whole pipeline vectorizes cleanly. Stack the $n$ queries, keys, and values as rows of matrices:
