...
This is the **dot-product attention score**. The scalar $\beta^2$ is just a constant; in practice it is set to $1/\sqrt{d}$, where $d$ is the dimension of the query and key vectors. The reason is statistical. Suppose the components of $\boldsymbol{q}$ and $\boldsymbol{k}_i$ are independent, with mean zero and unit variance. Each product term $q_\ell\, k_{i,\ell}$ is then also mean-zero, with
