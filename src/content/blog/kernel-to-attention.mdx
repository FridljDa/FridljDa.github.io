...
This is mostly a rename: $\boldsymbol{k}_i = \boldsymbol{x}_i$ and $\boldsymbol{v}_i = y_i$. The split earns its keep in §8, where learned projections will let keys and values diverge.
...
This is the **dot-product attention score**. It tells us the *form* of the similarity — but not its magnitude. The scalar $\beta^2$ is still a free parameter; §5 determines its standard value.

---

## 5. Scaling: why divide by $\sqrt{d}$

In practice $\beta^2 = 1/\sqrt{d}$, where $d$ is the dimension of the query and key vectors. The reason is statistical. Suppose the components of $\boldsymbol{q}$ and $\boldsymbol{k}_i$ are independent, with mean zero and unit variance. Each product term $q_\ell\, k_{i,\ell}$ is then also mean-zero, with
...
This is the step that turns nonparametric kernel regression into a *trained* attention mechanism. The key/value split from §3 now earns its keep: because $W^{(k)}$ and $W^{(v)}$ are independent matrices, a token can be matched on one aspect of itself and return another.
