...
This is mostly a rename: $\boldsymbol{k}_i = \boldsymbol{x}_i$ and $\boldsymbol{v}_i = y_i$. The split earns its keep in §6, where learned projections will let keys and values diverge.
...
But something has collapsed. The §3 table distinguished **keys** (what the query is compared against) from **values** (what gets averaged into the output), yet in (14) we have $\boldsymbol{k}_j = \boldsymbol{v}_j = \boldsymbol{x}_j$ — literally the same vector. A token must be matched on exactly the same information it contributes. That's the restriction §6 removes.

---

## 6. Making it parametric: learned projections
