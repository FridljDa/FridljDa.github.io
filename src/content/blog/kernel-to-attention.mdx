...
\xrightarrow{\text{Gaussian kernel}} \quad & \underbrace{\sum_i \operatorname{softmax}_i\!\left(-\tfrac{\beta^2}{2}\|\boldsymbol{x} - \boldsymbol{x}_i\|^2\right) y_i}_{\text{nonparametric attention}} \\[4pt]
...
\xrightarrow{\text{expand, drop constants}} \quad & \underbrace{\operatorname{softmax}\!\left(\tfrac{Q K^\top}{\sqrt{d}}\right) V}_{\text{scaled dot-product}} \\[4pt]
\xrightarrow{\text{learned projections + self-query}} \quad & \underbrace{\boldsymbol{y}_i = \operatorname{Attn}(W^{(q)} \boldsymbol{x}_i, W^{(k)} X, W^{(v)} X)}_{\text{self-attention}}
\end{aligned}
$$
