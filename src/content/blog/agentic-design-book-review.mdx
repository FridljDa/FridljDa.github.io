...
The chapter should have mentioned that reasoning models have absorbed the classic self-critique loop in many settings.
...
Useful concept to consider for advanced agentic systems. If we know the steps needed to solve a problem in advance, planning should be avoided. Routing can help here.
Cursor's "Plan Mode" is an excellent example of a use case where planning can be useful.

## Chapter 7: Multi-Agent 

This is a very active area of research. A lot has been tried since the book was published. The chapter reads very thin and misses much of what the community has learned about multi-agent systems since then.
Cursor's experiments with self-driving codebases are particularly noteworthy: https://cursor.com/blog/self-driving-codebases
...
Some concepts would be implemented in Pydantic AI via https://ai.pydantic.dev/dependencies/, which is more general than memory management but can be used for it as well.
